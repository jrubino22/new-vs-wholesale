...
//       this.link.addEventListener('focus', this.showDisclosure.bind(this));
  ...
  const selectors$u = {
    ...
    increaseCart: '[data-increase-quantity-cart]',
    decreaseCart: '[data-decrease-quantity-cart]',
  ...
  class Quantity {
    constructor(wrapper) {
      ...
      this.increaseCart = this.wrapper.querySelector(selectors$u.increaseCart);
      this.decreaseCart = this.wrapper.querySelector(selectors$u.decreaseCart);
    ...
    initButtons() {
      ...
      if(this.decrease){
        this.decrease.addEventListener(
          ...
          function (e) {
            ...
            if(this.increaseCart){
        this.increaseCart.addEventListener(
          'click',
          function (e) {    
            let v = parseInt(this.input.value, 10);
            v = isNaN(v) ? 0 : v;
      ...
      if(this.decreaseCart){
        this.decreaseCart.addEventListener(
          'click',
          function (e) {            
            let v = parseInt(this.input.value, 10);
            v = isNaN(v) ? 0 : v;
  ...
  class CartItems {
    ...
    updateTotal() {
      ...
      if (this.cart && this.cart.total_price) {
        ...
        $("span.wcp-original-cart-total").text(price);
        if(this.finalPrice) this.finalPrice.innerHTML = price;
      }
      let priceTotal = wcpcartdata.wcp_cart_total.toString().slice(0, -2);
      let priceLimit = $(".checkout_wrapper").data('limit');
      if(priceTotal >= priceLimit){
        $(".cart__footer__checkout_warning").fadeOut()
      	$(".checkout_wrapper").fadeIn()
      }else{
       $(".cart__footer__checkout_warning").fadeIn()
      	$(".checkout_wrapper").fadeOut()
      }
    }
    ...
    init() {
      this.button.addEventListener(
        ...
        function (evt) {
          ...
          if (this.form.classList.contains('regular_form')) {
           evt.preventDefault();
          }
          ...
          //     // this.form.querySelector('[type="file"]').setAttribute('data-ajax-disable', '')
          //   formData.append('line_item.properties', (this.form.querySelector('[type="file"]').value))
              
          // console.log(this.form.querySelector('[type="file"]').value)
          // }
...
Shopify.formatMoney = function (t, e) {
  ...
  return a.replace(n, r);
   ...
   $(".btn--custom-gc").click(function(){
     
     const mainProdQuant = parseInt($(".custom__qi").val());
     const colorProdId = parseInt($(".product_option_1").find(":selected").attr("value"));
     const sleeveId = parseInt($(".gc_sleeve_id_holder").val())
     const sleeveBool = $(".gc_sleeve_input").is(":checked");
     const EPId = parseInt($(".gc_EP_id_holder").val());
     const EPBool = $(".gc_EP_input").is(":checked")
     const serialId = parseInt($(".gc_serial_input").val());
     const serialBool = $(".gc_serial_input").is(":checked");
     const namesId = parseInt($(".gc_names_input").val());
     const namesBool = $(".gc_names_input").is(":checked");
     const stripsId = parseInt($(".gc_strips_input").val());
     const stripsBool = $(".gc_strips_input").is(":checked");
     const minValBox = parseInt($(".custom__qi").attr("min"));
     console.log("sl ID", sleeveId)


      let formDataColor =
        {
          "id": `${colorProdId}`,
          "quantity": `${mainProdQuant}`
        };
     ...
     let formDataEP = 
      ...
      function submitForm2() {              
         return fetch(`${window.theme.routes.cart}/add.js`, {
                        method:"POST",
                        credentials: "same-origin",
                        headers: {
                          "Content-Type": "application/json"
                        },
                        body: JSON.stringify(formDataColor)
                           }).then(function (data) {
    ...
    function submitForm3() {
       if (EPBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataEP)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm4() {
       if (sleeveBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataSleeve)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm7() {
       if (stripsBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataStrips)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     async function sixForms(){
       await submitForm6()
       fiveForms()
     }     
     
     async function fiveForms(){
       await submitForm5()
       fourForms()
     }     

    async function fourForms(){
       await submitForm4()
       threeForms()
     }

     async function threeForms(){
       await submitForm3()
       twoForms()
     }

      async function twoForms() {
          await submitForm2();                
          $(".gc-form").submit();
      }

     console.log($('.gc_img_upload'). val())
  function gcValidation() {
    if (mainProdQuant >= minValBox && $('.gc_img_upload').get(0).files.length !== 0) {
       return allForms()
  } else if (mainProdQuant < minValBox) {
      alert(`The minimum order quantity for this product is ${minValBox}`)
  } else {
...
$(".btn--custom-smt").click(function(){
  
  const mainProdQuant = parseInt($(".custom__qi").val());
  const lidId = parseInt($(".LidId").val());
  const lidBool = $(".lid_check").is(":checked");
  const minValBox = parseInt($(".custom__qi").attr("min"));
  let formDataLids = 
        {
          "id": `${lidId}`,
       ...
       if (lidBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataLids)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
  ...
  function smtValidation() {
   ...
   $(".btn--custom-grinder").click(function(){
     
     const mainProdQuant = parseInt($(".custom__qi").val());
     const plushId = parseInt($(".plush-bag").attr("data-prod-id"));
     const plushBool = $(".plush-bag").is(":checked");
     
     const fullColorId = parseInt($(".full_color_select").find(":selected").attr("data-prod-id"));  
     const fullColorBool = $(".fcp-check").is(":checked");
     
     const glowColorId = parseInt($(".glow_color_select").find(":selected").attr("data-prod-id"));  
     const glowColorBool = $(".gitd-check").is(":checked");
     
     const rubPaintId = parseInt($(".rub-paint").attr("data-prod-id"));
     const rubPaintBool = $(".rub-paint").is(":checked")
     
     const multiPaintId = parseInt($(".multi-paint").attr("data-prod-id"));
     const multiPaintBool = $(".multi-paint").is(":checked");
     
     const nonStickId = parseInt($(".non-stick").attr("data-prod-id"));
     const nonStickBool = $(".non-stick").is(":checked");

     const minValBox = parseInt($(".custom__qi").attr("min"));
     console.log("sl ID", fullColorId)
     ...
     let formDataFullColor = 
        {
          "id": `${fullColorId}`,
          "quantity": `${mainProdQuant}`
        };
     
      let formDataGlowColor = 
        {
          "id": `${glowColorId}`,
          "quantity": `${mainProdQuant}`
        };
      let formDataRubPaint = 
        {
          "id": `${rubPaintId}`,
          "quantity": `${mainProdQuant}`
        };
      let formDataMultiPaint = 
        {
          "id": `${multiPaintId}`,
          "quantity": `${mainProdQuant}`
        };
      let formDataNonStick = 
        {
          "id": `${nonStickId}`,
          "quantity": `${mainProdQuant}`
        };     
      ...
      function submitForm2() {          
        if(plushBool) {
         return fetch(`${window.theme.routes.cart}/add.js`, {
                        ...
                        body: JSON.stringify(formDataPlush)
                           }).then(function (data) {
                            if (Response.status == 200) {
                              window.location.reload;
                              return
    ...
    function submitForm3() {
       if (fullColorBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataFullColor)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm4() {
       if (glowColorBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataGlowColor)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm5() {
       if (rubPaintBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataRubPaint)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm6() {
       if (multiPaintBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataMultiPaint)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
     ...
     function submitForm7() {
       if (nonStickBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataNonStick)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
 ...
 $(".btn--custom-bong").click(function(){
     
     const mainProdQuant = parseInt($(".custom__qi").val());
     const minValBox = parseInt($(".custom__qi").attr("min"));
   
     const decalId = parseInt($(".decal-check").attr("data-prod-id"));
     const decalBool = $(".decal-check").is(":checked");

     const TBColorId = parseInt($(".color-check").attr("data-prod-id"));
     const TBColorBool = $(".color-check").is(":checked");
      ...
      function submitForm2() {          
        if(TBColorBool) {
         return fetch(`${window.theme.routes.cart}/add.js`, {
                        ...
                        body: JSON.stringify(formDataTBColor)
                           }).then(function (data) {
                            if (Response.status == 200) {
                              window.location.reload;
                              return
    ...
    function submitForm3() {
       if (decalBool) {
          return fetch(`${window.theme.routes.cart}/add.js`, {
                          ...
                          body: JSON.stringify(formDataAddDecal)
                             }).then(function (data) {
                              if (Response.status == 200) {
                                window.location.reload;
                                return
